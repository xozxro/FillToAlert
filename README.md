...
By bypassing the image verifaction on Webull after saving the DID created on your initial load of the Webull page, you have logged that DID as having succesfully completed that verification to Webull. So, we need to pass this ID with your credentials, so that Webull doesn't require image verification when accessing the account via this script - which we cannot handle progrmatically.
